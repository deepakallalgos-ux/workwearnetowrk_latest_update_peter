...
class pjShoppingCart
{
	...
	{
		return $this->model->reset()->where('t1.is_cart', '1')->where('t1.hash', (is_null($hash) ? $this->hash : $hash))->findAll()->getData();
	}

	public function has($key)
	{
		if ($this->model->reset()->where('t1.is_cart', '1')->where('t1.hash', $this->hash)->where('t1.key_data', $key)->findCount()->getData() == 0) {
			return false;
		}
	...
	public function isEmpty()
	{
		...
		if ($this->model->reset()->where('t1.is_cart', '1')->where('t1.hash', $this->hash)->findCount()->getData() == 0) {
			return true;
		}

...
class pjBrandModel extends pjAppModel
{
	...
	{
		$default_company = pjUtil::getSessionCompany($this->defaultCompany);
		...
		if (count($arr) === 1) {
			...
			$q = $this->reset()
				...
				->join(
					'pjMultiLang',
					"t2.foreign_id = t1.id AND t2.model = 'pjBrand' AND t2.locale = '$locale_id' AND t2.field = 'name'",
					'left outer'
				)
				->where(sprintf("t1.lft BETWEEN '%u' AND '%u'", $arr[0]['lft'], $arr[0]['rgt']));

			// add company filter only if valid
			if (isset($default_company['id']) && $default_company['id'] !== '') {
				$q->where('t1.company_id', $default_company['id']);
			}
			...
			foreach ($descendants as $descendant) {
				...
				// }
				if (!empty($right)) {
					while (!empty($right) && end($right) < $descendant['rgt']) {
						array_pop($right);
					}
				}
	...
	public function rebuildTree($parent_id, $left)
	{
		...
		// ✔ apply company_id only if valid
		if (isset($default_company['id']) && $default_company['id'] !== '') {
			$q->where('t1.company_id', $default_company['id']);
		}

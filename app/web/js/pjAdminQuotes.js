...
(function ($, undefined) {
	$(function () {
		...
		function getStockProducts(quote_id) {
			$.get("index.php?controller=pjAdminQuotes&action=pjActionStockGet", {
				"quote_id": quote_id
			}).done(function (data) {
		...
		if ($frmUpdateQuote.length > 0) {
			$frmUpdateQuote.on("change", "#client_id", function () {
			...
			if (validate) {
				$frmUpdateQuote.validate({
					rules: {
						"uuid": {
							remote: "index.php?controller=pjAdminQuotes&action=pjActionCheckUID&id=" + $frmUpdateQuote.find("input[name='id']").val()
						}
					},
					messages: {
						"uuid": {
			...
			getStockProducts.call(null, $frmUpdateQuote.find("input[name='id']").val());
		}
		...
		function formatUuid(val, obj) {
			return ['<a href="index.php?controller=pjAdminQuotes&action=pjActionUpdate&id=', obj.id, '">', val, '</a>'].join("");
		}
		function formatStatus(val, obj) {
			var statusKey = (obj.status !== undefined && obj.status !== null && obj.status !== '') ? String(obj.status) : (val != null ? String(val) : 'new');
			var label = obj.status_formated || val || statusKey;
			return '<span class="pj-table-cell-label pj-status-' + statusKey + '">' + label + '</span>';
		}
		if ($("#grid").length > 0 && datagrid) {
			...
			if (myLabel.has_update) {
				$editable = true;
				$buttons.push({type: "edit", url: "index.php?controller=pjAdminQuotes&action=pjActionUpdate&id={:id}"});
			}
			if (myLabel.has_delete) {
				$buttons.push({type: "delete", url: "index.php?controller=pjAdminQuotes&action=pjActionDeleteQuote&id={:id}"});
			}
			if (myLabel.has_delete_bulk) {
				$actions.push({text: myLabel.delete_selected, url: "index.php?controller=pjAdminQuotes&action=pjActionDeleteQuoteBulk", render: true, confirmation: myLabel.delete_confirmation});
			}
			// $actions.push({text: myLabel.exported, url: "index.php?controller=pjAdminQuotes&action=pjActionExportQuote", ajax: false});
			if ($actions.length > 0) {
				$select = {
						field: "id",
						name: "record[]",
			...
			var $grid = $("#grid").datagrid({
				buttons: $buttons,
				columns: [{text: myLabel.uuid, type: "text", sortable: true, editable: false, renderer: formatUuid},
				          ...
				          {text: myLabel.total, type: "text", sortable: true, renderer: formatTotal},
				          {text: myLabel.status, type: "text", sortable: true, editable: false, renderer: formatStatus, cellClass: 'col-orders-status'}
				       ],
				dataUrl: "index.php?controller=pjAdminQuotes&action=pjActionGetQuote" + pjGrid.queryString,
				dataType: "json",
				...
				saveUrl: "index.php?controller=pjAdminQuotes&action=pjActionSaveQuote&id={:id}",
				select: $select
			});			
		}
		
		if ($("#grid_client_quotes").length > 0 && datagrid) {
			var $buttons = [];
			var $actions = [];
			var $editable = true;
			var $select = false;
			...
			var $grid_client_quotes = $("#grid_client_quotes").datagrid({
				buttons: $buttons,
				...
				dataUrl: "index.php?controller=pjAdminQuotes&action=pjActionGetQuote&is_client_quotes=1&client_id=" + $("#client_id").find("option:selected").val() + "&quote_id=" + $("input[name='id']").val(),
				dataType: "json",
		...
		$(document).on("change", "#client_id", function () {
			...
			if (parseInt(client_id, 10) > 0) {
				$.get("index.php?controller=pjAdminQuotes&action=pjActionGetClient", {
					"client_id": client_id
				}).done(function (data) {
					$("#boxClient").html(data);
				});
				$.get("index.php?controller=pjAdminQuotes&action=pjActionGetAddressBook", {
					"client_id": client_id,
					"quote_id": $("input[name='id']").val()
				}).done(function (data) {
			...
			var content = $grid_client_quotes.datagrid("option", "content"),
				cache = $grid_client_quotes.datagrid("option", "cache"),
				obj = {};
			obj.is_client_quotes = 1;
			obj.quote_id = $("input[name='id']").val();
			obj.client_id = parseInt(client_id, 10) > 0 ? client_id : 0;
			
			$.extend(cache, obj);
			$grid_client_quotes.datagrid("option", "cache", cache);
			$grid_client_quotes.datagrid("load", "index.php?controller=pjAdminQuotes&action=pjActionGetQuote", "id", "DESC", content.page, content.rowCount);
			
		}).on("change", "#address_id", function () {
			if (parseInt($("option:selected", this).val(), 10) > 0) {
				$(".btnCopy").prop("disabled", false);
				$.get("index.php?controller=pjAdminQuotes&action=pjActionGetAddress", {
			...
			var $quote_stock_id = $(this).attr('data-id');
			var $stockEditContentWrapper = $('#stockEditContentWrapper');	
			$stockEditContentWrapper.html("");
			$.get("index.php?controller=pjAdminQuotes&action=pjActionStockEdit", {
			...
			$('#btnStockAddConfirm').attr('data-id', $quote_id);			
			$stockAddContentWrapper.html("");
			$.get("index.php?controller=pjAdminQuotes&action=pjActionStockAdd", {
				"quote_id": $quote_id
			}).done(function (data) {
			...
			swal({
				...
				$.post("index.php?controller=pjAdminQuotes&action=pjActionStockDelete", {id: id}).done(function (data) {
					swal.close();
					getStockProducts.call(null, $frmUpdateQuote.find("input[name='id']").val());
				});
			});
		}).on("click", ".quote-calc", function () {
			var $this = $(this),
				$form = $this.closest("form");
			$.post("index.php?controller=pjAdminQuotes&action=pjActionGetPrice", $form.serialize()).done(function (data) {
				if (data.status == 'OK') {
		...
		}).on("click", "#btnEmailQuote", function (e) {
			if (e && e.preventDefault) {
				e.preventDefault();
			}
			var $id = $(this).attr('data-id'),
			...
			$.get("index.php?controller=pjAdminQuotes&action=pjActionQuoteEmail", {
				"id": $id
			}).done(function (data) {
			...
			if (validator.form()) {
				...
				var l = Ladda.create(this);
			 	l.start();
				$.post("index.php?controller=pjAdminQuotes&action=pjActionQuoteEmail", $confirmEmailContentWrapper.find("form").serialize()).done(function (data) {
					if (data.status == "OK") {
			...
			$.get("index.php?controller=pjAdminQuotes&action=pjActionPaymentEmail", {
				"id": $id
			}).done(function (data) {
			...
			if (validator.form()) {
				...
				var l = Ladda.create(this);
			 	l.start();
				$.post("index.php?controller=pjAdminQuotes&action=pjActionPaymentEmail", $paymentEmailContentWrapper.find("form").serialize()).done(function (data) {
					if (data.status == "OK") {
		...
		$(document).on("change", ".stock-product", function () {
			var $this = $(this);
			$.get("index.php?controller=pjAdminQuotes&action=pjActionGetStocks", {
				...
				$.get("index.php?controller=pjAdminQuotes&action=pjActionStockGetByProduct", {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminQuotes&action=pjActionGetQuote", "id", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("index.php?controller=pjAdminQuotes&action=pjActionStockAdd", qs).done(function (data) {
				getStockProducts.call(null, $frmUpdateQuote.find("input[name='id']").val());
			...
			var $this = $(this);
			$.post("index.php?controller=pjAdminQuotes&action=pjActionStockEdit", $('#stockEditModal').find("form").serialize()).done(function (data) {
				getStockProducts.call(null, $frmUpdateQuote.find("input[name='id']").val());

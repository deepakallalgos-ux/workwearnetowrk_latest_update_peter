...
class pjMollie extends pjMollieAppController
{
	...
	public static function getRetryPaymentUrl($order)
	{
		$hash = md5($order['uuid'] . $order['created'] . PJ_SALT);
		$base_url = rtrim(PJ_INSTALL_URL, '/');

		return $base_url . '/index.php?controller=pjMollie&action=pjActionRetryPayment&uuid=' . urlencode($order['uuid']) . '&hash=' . urlencode($hash);
	}
}
?>
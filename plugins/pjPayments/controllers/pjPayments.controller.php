...
class pjPayments extends pjPaymentsAppController
{
        ...
        if ($this->_post->check('options_update')) {
            ...
            if (in_array($this->_post->toString('payment_method'), array('cash', 'bank'))) {
                pjPaymentOptionModel::factory()
                    ...
                    ->limit(1)
                    ->modifyAll(array(
                        'is_active' => $this->_post->toInt('is_active'),
                        'company_id' => $company_id
                    ));
            }
    ...
    //         $log('--- PAYMENT OPTIONS SAVE START ---');
    //         $log('POST = ' . print_r($this->_post, true));

    //         $foreign_id = $this->getForeignId();
    //         $payment_method = $this->_post->toString('payment_method');
    //         $is_active = $this->_post->toInt('is_active');
    ...
    //         /* ========== ONLINE PAYMENT OPTIONS (PLUGINS) ========== */
    //         if (
    //             pjObject::getPlugin('pjPayments') !== NULL &&
    //             $this->_post->check('plugin_payment_options')
    //         ) {
    //             $log('Saving ONLINE payment plugin options');
    ...
    //             $log('Saving OFFLINE payment method');

    //             $affected = pjPaymentOptionModel::factory()
    ...
    //             $log('DB update done, affected rows = ' . (int)$affected);
    //         }

    //         /* ========== PAYMENT LABEL (MULTI-LANG) ========== */
    //         if ($this->_post->check('i18n')) {
    //             $log('Saving payment labels (i18n)');

    //             pjMultiLangModel::factory()->updateMultiLang(
    //                 $this->_post->toI18n('i18n'),
    //                 $foreign_id,
    //                 'pjPayment',
    //                 'data'
    //             );
    ...
    //         /* ========== OPTION LABELS (MULTI-LANG) ========== */
    //         if ($this->_post->check('i18n_options')) {
    //             $log('Saving option labels (i18n_options)');

    //             pjMultiLangModel::factory()->updateMultiLang(
    //                 $this->_post->toI18n('i18n_options'),
    //                 $foreign_id,
    //                 'pjOption',
    //                 'data'
    //             );
    ...
    //         $log('--- PAYMENT OPTIONS SAVE END ---');
    //     } else {

    //         /* ========== LOAD DATA FOR MODAL ========== */
    //         $log('Loading payment options modal');

    //         $this->set(
    //             'i18n',
    //             pjMultiLangModel::factory()->getMultiLang(
    //                 $this->getForeignId(),
    //                 'pjPayment'
    //             )
    //         );
    ...
    public function pjActionSaveOptions()
    {
        ...
        $have_integration_plugin = pjObject::getPlugin('pjIntegration') !== NULL && class_exists('pjIntegrationModel');
        ...
        foreach ($params['data'] as $payment_method => $data) {
            ...
            if ($have_integration_plugin && array_key_exists('is_active', $data)) {
                pjIntegrationModel::factory()
                    ->where('slug', $payment_method)
                    ->limit(1)
                    ->modifyAll(array(

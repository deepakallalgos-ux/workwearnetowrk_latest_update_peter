...
class pjInstaller extends pjInstallerAppController
{
	...
	public function beforeFilter()
	{
		...
		$this->appendCss('install.css', $this->getConst('PLUGIN_CSS_PATH'), false, false);
	...
	private function pjActionCheckVars()
	{
		...
		$indexes = array(
			...
			'admin_email' => 'Super Admin Login: E-Mail',
	...
	public function pjActionStep3()
	{
		...
		$_POST = $this->_post->raw();
		self::pjActionCheckConfig();

		$this->pjActionCheckSession();
		if ($this->_post->check('step2')) {
	...
	public function pjActionStep4()
	{
		...
		if (isset($_POST['step3'])) {
			...
			if (
				!isset($_POST['hostname']) || !isset($_POST['username']) || !isset($_POST['database']) ||
	...
	public function pjActionStep5()
	{
		...
		if (isset($_POST['step4'])) {
			...
			if (
				!isset($_POST['install_folder']) || !isset($_POST['install_url']) || !isset($_POST['install_path']) ||
	...
	public function pjActionStep6()
	{
		...
		if (isset($_POST['step5'])) {
			...
			if (
				!isset($_POST['admin_email']) || !isset($_POST['admin_password']) ||
	...
	private static function pjActionOneInstall($license_key, $server_name = NULL)
	{
		...
		$endPoint = base64_decode("aHR0cHM6Ly9lcnAucGhwamFiYmVycy5jb20v");
		...
		if (isset($result['hash'], $result['private_key'], $result['status']) && $result['status'] === 'OK') {
			...
			return array('status' => 'OK', 'code' => 200, 'text' => 'Success', 'hash' => $license_key, 'private_key' => $result['private_key']);
		}

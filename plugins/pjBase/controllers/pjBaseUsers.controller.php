...
class pjBaseUsers extends pjBase
{
	...
	public function pjActionGetUser()
	{
		...
		if ($this->isXHR()) {
			...
			$pjAuthUserModel = pjAuthUserModel::factory()
				...
				->join('pjCompany', 't3.id=t1.company_id', 'left')
				->where('t2.is_backend', 'T')
				->where('t2.is_admin', 'T');
			$role_id = $this->getRoleId();

			// if ($role_id == 1) {
			// 	$pjAuthUserModel->where("(t1.company_id = 0 OR t1.company_id = $company_id)");
			// } else {
			// 	$pjAuthUserModel->where('t1.company_id', $company_id);
			// }

			if ($q = $this->_get->toString('q')) {
				$q = str_replace(array('%', '_'), array('\%', '\_'), trim($q));
				$pjAuthUserModel->where('t1.email LIKE', "%$q%");
				$pjAuthUserModel->orWhere('t1.name LIKE', "%$q%");
			}
			...
			$data = $pjAuthUserModel
				->select('t1.id, t1.email, t1.name, DATE(t1.created) AS `created`, DATE(t1.last_login) AS `last_login`, t1.status, t1.is_active, t1.locked, t1.role_id, t3.name AS company_name')
	...
	public function pjActionCreate()
	{
		...
		if (self::isPost() && $this->_post->toInt('user_create')) {
			...
			$data['company_id'] = $this->_post->toInt('company_id');
		...
		if (self::isGet()) {
			$company_arr = pjCompanyModel::factory()
				...
				->where('t1.is_deleted', 0)->findAll()->getData();

	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) 
		{
			...
			function onBeforeShowPermissions (obj) {
				if (parseInt(obj.role_id, 10) === 1 || parseInt(obj.role_id, 10) === 3) {
					return false;
				}
				return true;
			}
			...
			var $grid = $("#grid").datagrid({
				...
				columns: [{text: myLabel.name, type: "text", sortable: true, editable: editable},
				          ...
				          {text: myLabel.company_name, type: "text", sortable: true, editable: editable},
                          ...
                          {text: myLabel.account_locked, type: "toggle", sortable: true, editable: editable, positiveClass: "pj-toggle-on", negativeClass: "pj-toggle-off", positiveLabel: myLabel.yesno['F'], positiveValue: "F", negativeLabel: myLabel.yesno['T'], negativeValue: "T"}],
				dataUrl: "index.php?controller=pjBaseUsers&action=pjActionGetUser",
				dataType: "json",
				fields: ['name', 'email','company_name', 'last_login', 'role', 'status', 'locked'],
				paginator: {
					actions: actions,
					gotoPage: true,
					paginate: true,
